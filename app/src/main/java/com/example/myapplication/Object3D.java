...
public class Object3D {
	...
	private float uvShift = 0.0f;
	// Our texture buffer.
	private FloatBuffer texcoordBuffer;
	private ArrayList<Float> tlist;
	...
	public void shiftUVsH(float shift) {
		...
		texcoordBuffer.position(0);
		for (int j = 0; j < tindex.size(); j++) {
			float adjustedU = tlist.get(tindex.get(j) * 2) + uvShift;  // Ajusta U en lugar de V
			float adjustedV = tlist.get(tindex.get(j) * 2 + 1);
